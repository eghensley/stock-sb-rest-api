...
public class PriceController {
	...
	@ApiOperation(value = "Add new price")
	@PostMapping("instrument/{instrumentOid}/add")
	public ResponseEntity<GeneralApiResponse> addNewInstrument(@PathVariable("instrumentOid") String instrumentOid,
			@RequestBody BasicPriceDto request) {
		GeneralApiResponse response = priceService.addNewPrice(instrumentOid, request);
		return new ResponseEntity<>(response, response.getStatus());
	}
	
	@ApiOperation(value = "Fetch latest saved price for instrument")
	@GetMapping("instrument/{instrumentOid}/type/{type}/latest")
	public ResponseEntity<GetResponse> getInstrumentBySymbol(@PathVariable("instrumentOid") String instrumentOid, @PathVariable("type") PriceTypeEnum type) {
